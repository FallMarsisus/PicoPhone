...
void pika_lvgl_img_dsc_t___init__(PikaObj* self, PikaObj* dsc_dict) {
    ...
    uint8_t* data = pikaDict_getBytes((PikaDict*)dsc_dict, "data");
    ...
    lv_img_dsc_t img_dsc = {
        .data = data,
        .data_size = pikaDict_getInt((PikaDict*)dsc_dict, "data_size"),
        .header =
            {
                .always_zero = 0,
...
void pika_lvgl_chart___init__(PikaObj* self, PikaObj* parent) {
    ...
    obj_runNativeMethod(obj_getObj(self, "TYPE"), "__init__", NULL);
    obj_runNativeMethod(obj_getObj(self, "AXIS"), "__init__", NULL);
    obj_runNativeMethod(obj_getObj(self, "UPDATE_MODE"), "__init__", NULL);
    obj_runNativeMethod(obj_getObj(self, "DRAW_PART"), "__init__", NULL);
}
...
void pika_lvgl_keyboard___init__(PikaObj* self, PikaObj* parent) {
    ...
    obj_runNativeMethod(obj_getObj(self, "MODE"), "__init__", NULL);
}
...
void pika_lvgl_ime_pinyin___init__(PikaObj* self, PikaObj* parent) {
    if (NULL != parent) {
#if defined(LV_USE_IME_PINYIN) && LV_USE_IME_PINYIN
        lv_obj_t* lv_obj = lv_ime_pinyin_create(PIKA_LV_OBJ(parent));
#else
        lv_obj_t* lv_obj = lv_obj_create(PIKA_LV_OBJ(parent));
#endif
        PIKA_LV_OBJ_SET(self, lv_obj);
    }
}
