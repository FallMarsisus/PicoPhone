...
// Déclaration de la fonction de pont C++ vers C
extern void pika_app_go_home(void);
...
PIKA_WEAK void user_lvgl_init(void) {}
...
void pika_lvgl_deinit(PikaObj* self) {
    if (g_lvgl_inited) {
        g_lvgl_inited = 0;
        pks_eventListener_deinit(&g_pika_lv_timer_event_listener);
    }
}
