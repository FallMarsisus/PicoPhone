...
private:
    ...
    static void pushCmtiIdx(int idx)
    {
        ...
        if (next == s_cmti_q_head)
            return; // queue pleine, index perdu
        s_cmti_q[s_cmti_q_tail] = (int8_t)idx;
        s_cmti_q_tail = next;
    }
    static bool popCmtiIdx(int &out)
    {
        if (s_cmti_q_head == s_cmti_q_tail)
            return false;
    ...
    static String sendAT(const String &cmd, unsigned long timeout_ms = 1500)
    {
            ...
            if (c == '\n')
            {
                ...
                if (s_line_buf.length() > 0)
                {
                    if (s_line_buf.startsWith("+CMTI:"))
                    {
                        ...
                        if (comma != -1)
                        {
                            String idx = s_line_buf.substring(comma + 1);
                            idx.trim();
        ...
        Serial1.print('\r');
        sleep_ms(50); // 20ms c'est souvent trop court pour le A7670E !
    ...
    {
        if (op.indexOf("Orange") != -1 || op.indexOf("Sosh") != -1)
            return "orange";
        if (op.indexOf("SFR") != -1 || op.indexOf("RED") != -1)
            return "sl2sfr";
        if (op.indexOf("Bouygues") != -1 || op.indexOf("B&You") != -1)
            return "mmsbouygtel.com";
        if (op.indexOf("Free") != -1)
            return "free";
        return "internet";
    }
    ...
    static void recoverModemIfNeeded(const char *reason)
    {
        ...
        if (now - s_last_recover_ms < 30000UL && s_last_recover_ms != 0)
            return;
    ...
    {
        if (num == "20801" || num == "20802")
            return "Orange";
        if (num == "20810" || num == "20811" || num == "20813")
            return "SFR";
        if (num == "20815")
            return "Free Mobile";
        if (num == "20816")
            return "NRJ Mobile";
        if (num == "20820" || num == "20821" || num == "20888")
            return "Bouygues";
        if (num == "26201")
            return "Telekom";
        if (num == "26202")
    ...
    static void pushIncomingSms(const String &number, const String &text, long ts)
    {
        ...
        strncpy(e.number, clean_number.c_str(), sizeof(e.number) - 1);
        e.number[31] = '\0';
        strncpy(e.text, clean_text.c_str(), sizeof(e.text) - 1);
        e.text[280] = '\0';
        e.timestamp = ts;
        s_sms_q_tail = next;
    }
    ...
    static void dispatchLine(const String &line)
    {
        ...
        if (line.startsWith("+CREG:") || line.startsWith("+CGREG:"))
        {
            ...
            else
            {
                ...
                if (urc_col != -1)
                {
                    String sv = line.substring(urc_col + 1);
                    sv.trim();
                    stat = sv.toInt();
                }
            }
            ...
            {
                s_registered = true;
                if (operator_name == "Recherche..." || operator_name == "Aucun service")
                    last_check = 0;
            }
...
public:
    ...
    static void powerOnA7670E()
    {
        ...
        sleep_ms(1500); // 1.5s est recommandé pour le A7670E
        ...
        sleep_ms(4000); // Un peu plus long pour s'assurer que la carte SIM est lue
    ...
    static const char *getErrorString(NetworkErrorCode code)
    {
        switch (code)
        {
        case NetworkErrorCode::NO_ERROR:
            return "OK";
        case NetworkErrorCode::MODEM_NOT_RESPONDING:
            return "Modem non réactif";
        case NetworkErrorCode::NO_SIGNAL:
            return "Pas de signal";
        case NetworkErrorCode::HTTP_TIMEOUT:
            return "Timeout réseau";
        case NetworkErrorCode::HTTP_NETWORK_ERROR:
            return "Erreur réseau";
        case NetworkErrorCode::HTTP_INVALID_RESPONSE:
            return "Réponse invalide";
        case NetworkErrorCode::SMS_SEND_FAILED:
            return "SMS non envoyé";
        case NetworkErrorCode::TIME_SYNC_FAILED:
            return "Sync heure échouée";
        case NetworkErrorCode::MODEM_CRASH:
            return "Modem crashé!";
        case NetworkErrorCode::REGISTRATION_FAILED:
            return "Enregistrement échoué";
        case NetworkErrorCode::DATA_CONNECTION_FAILED:
            return "Connexion données échouée";
        case NetworkErrorCode::RECOVERY_IN_PROGRESS:
    ...
    {
        if (s_sms_q_head == s_sms_q_tail)
            return false;
        const SmsEnvelope &e = s_sms_q[s_sms_q_head];
        if (out_number)
            strncpy(out_number, e.number, 31);
        if (out_text)
            strncpy(out_text, e.text, 280);
        if (out_ts)
            *out_ts = e.timestamp;
        s_sms_q_head = (s_sms_q_head + 1) % SMS_Q_SIZE;
        return true;
                                   ...
                                   const String &extraHeaders = "")
    {
        ...
        for (int attempt = 1; attempt <= 2; attempt++)
        {
            ...
            {
                if (sendAT("AT+HTTPPARA=\"CONTENT\",\"" + contentType + "\"", 2000).indexOf("OK") == -1)
                {
                    sendAT("AT+HTTPTERM", 500);
                    continue;
                }
            ...
            while (millis() - data_start < 8000)
            {
                watchdog_update();
                while (Serial1.available())
                    data_resp += (char)Serial1.read();
                if (data_resp.indexOf("DOWNLOAD") != -1)
                    break;
                if (data_resp.indexOf("ERROR") != -1)
                    break;
                sleep_ms(10);
            }
            ...
            while (millis() - data_ok_start < 12000)
            {
                watchdog_update();
                while (Serial1.available())
                    data_ok += (char)Serial1.read();
                if (data_ok.indexOf("OK") != -1 || data_ok.indexOf("ERROR") != -1)
                    break;
                sleep_ms(10);
            }

            if (data_ok.indexOf("OK") == -1)
            {
                sendAT("AT+HTTPTERM", 500);
                continue;
            }

            Logger::println("[LTE->GSM] AT+HTTPACTION=1");
            ...
            if (http_code < 200 || http_code >= 300 || data_len < 0)
            {
                sendAT("AT+HTTPTERM", 500);
                continue;
            }
            ...
            if (data_len > MAX_HTTP)
            {
                Logger::printf("[LTE HTTP] ATTENTION: Reponse trop grande. Troncature a %d.\n", MAX_HTTP);
                data_len = MAX_HTTP;
            }
    ...
    static void update()
    {
        ...
        if (state == 0 && !s_sms_send_pending)
        {
            ...
            if (popCmtiIdx(cmti_idx))
            {
                ...
                if (cmgr_pos != -1)
                {
                    ...
                    if (nl1 != -1)
                    {
                        String hdr = resp.substring(cmgr_pos + 6, nl1);
                        hdr.trim();
                        String number = csvField(hdr, 1);
                        number.trim();
                        int text_start = nl1 + 1;
                        while (text_start < (int)resp.length() && resp[text_start] == '\r')
                            text_start++;
        ...
        if (state == 1)
            state_timeout = 45000UL; // 45 sec pour la recherche d'opérateur
        else if (state == 3)
            state_timeout = 5000UL;
        else if (state == 5)
            state_timeout = 15000UL; // Le serveur NTP peut mettre 10 secondes à répondre
        ...
        {
            if (time_sync_fail_count < 12)
                time_sync_fail_count++;
